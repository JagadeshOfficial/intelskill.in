...
export default function AdminTutorsPage() {
    ...
    useEffect(() => {
        ...
        const fetchTutors = async () => {
            try {
                setLoading(true);
                const res = await fetch(`${API_BASE_URL}/api/v1/auth/admin/tutors`, {
                    headers: { 'Authorization': `Bearer ${localStorage.getItem('adminToken')}` } // Add token if needed
                });
                if (!res.ok) {
    ...
    const handleStatusChange = async (tutorId: string, newStatus: string) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/v1/auth/admin/tutors/${tutorId}/status`, {
                method: 'PUT',
    ...
    const handleAddTutor = async () => {
        try {
            ...
            const res = await fetch(`${API_BASE_URL}/api/v1/tutor/register`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            const data = await res.json();
            // refresh list
            const listRes = await fetch(`${API_BASE_URL}/api/v1/auth/admin/tutors`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('adminToken')}` }
            });
